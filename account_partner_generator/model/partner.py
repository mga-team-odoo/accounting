...
class res_partner(orm.Model):
    ...
    def create_customer_account(self, cr, uid, ids, context=None, company_id=None):
        """
        ...
        if not company.acc_aux_customer_id:
            raise orm.except_orm(
                _('Error'),
                _('Not parent account defined for the customer on the company %s') % company.name)
        ...
        acctype_ids = account_type_obj.search(cr, uid, [('code', '=', 'receivable')], context=context)
        ...
        for cust in self.browse(cr, uid, ids, context=context):
            ...
            customer_account = seq_obj.next_by_code(cr, uid, 'account.account.customer', context=context)
            if '@' in customer_account:
                if '#' not in customer_account:
                    raise orm.except_orm(
                        _('Error'),
                        _('Missing # on customer sequence defined on the company %s') % company.name)
                customer_account = self._decode_sequence(cr, uid, customer_account, cust.ref)

            logger.info('Customer code %s' % (customer_account,))
            acc_ids = account_obj.search(
                cr, uid, [('company_id', '=', company_id), ('code', '=', customer_account)], context=context)
            if acc_ids:
                self.write(cr, uid, [cust.id],
                           {'property_account_receivable': acc_ids[0]}, context=context)
                continue
    ...
    def create_supplier_account(self, cr, uid, ids, context=None, company_id=None):
        """
        Create the customer account if not exists
        """
        company_obj = self.pool['res.company']
        ...
        if not company.seq_acc_supplier_id:
            raise orm.except_orm(
                _('Error'),
                _('No supplier sequence defined on the company %s') % company.name)

        acctype_ids = account_type_obj.search(cr, uid, [('code', '=', 'payable')], context=context)
        ...
        for supp in self.browse(cr, uid, ids, context=context):
            ...
            # We compose the code with prefix and sequence
            supplier_account = seq_obj.next_by_code(cr, uid, 'account.account.supplier', context=context)
            if '@' in supplier_account:
                if '#' not in supplier_account:
                    raise orm.except_orm(
                        _('Error'),
                        _('Missing # on supplier sequence defined on the company %s') % company.name)
                supplier_account = self._decode_sequence(cr, uid, supplier_account, supp.ref)

            logger.info('Supplier code %s' % (supplier_account,))
            acc_ids = account_obj.search(
                cr, uid, [('company_id', '=', company_id), ('code', '=', supplier_account)], context=context)
            if acc_ids:
                self.write(cr, uid, [supp.id],
                           {'property_account_payable': acc_ids[0]}, context=context)
                continue
